...
exports.extractSemanticIntent = extractSemanticIntent;
exports.scoreProduct = scoreProduct;
exports.rankAndFilterProducts = rankAndFilterProducts;
/**
 ...
 */
function extractSemanticIntent(query) {
...
function scoreProduct(product, intent) {
    ...
    const price = parseFloat(product.discount || ((_a = product.price) === null || _a === void 0 ? void 0 : _a.amount) || product.price || 0);
    ...
    for (const token of intent.normalizedKeywords) {
        if (['find', 'me', 'some', 'the', 'a', 'an', 'add', 'in', 'to', 'for', 'with', 'under', 'below'].includes(token))
            continue;
    ...
    if (product.stock && product.stock > 0)
        score += 5;
    if (product.stars)
        score += parseFloat(product.stars);
    return { product, score, price, matchReasons };
}
/**
 * 4. SCORE, RANK AND FILTER PRODUCTS
 */
function rankAndFilterProducts(products, query) {
