...
function classifyIntent(message, state) {
    ...
    // A. Checkout Intent
    if (/^(?:please\s+)?(?:proceed\s+to\s+)?checkout\b/i.test(lower) ||
    ...
    if (/^(what\s+(?:did\s+i|have\s+i)\s+(?:just\s+)?add(?:ed)?(?:\s+to\s+cart)?|what\s+was\s+added|show\s+(?:me\s+)?what\s+i\s+just\s+added)/i.test(lower)) {
        return { type: 'CHECK_LAST_ADDED', rawMessage: raw, referenceType: 'LAST_MENTIONED', confidence: 1.0 };
    }
    // D. View Cart / Cart Total
    if (/^(what('?s| is) in my cart|view (my )?cart|show (my )?cart|how much is my cart|cart summary|how many (items |products )?are in my cart|what is my total|check my cart)/i.test(lower)) {
    ...
    if (viewOrdinalMatch) {
        ...
        const ordMap = { 'first': 0, '1st': 0, 'second': 1, '2nd': 1, 'third': 2, '3rd': 2, 'fourth': 3, '4th': 3, 'last': -1 };
        return {
            type: 'VIEW_PRODUCT',
            rawMessage: raw,
    ...
    if (state.lastRecommendedProducts &&
        ...
        let extremum;
        if (/(?:cheapest|cheaper|lowest\s+price|least\s+expensive)/i.test(lower))
            extremum = 'cheapest';
        if (/(?:most\s+expensive|highest\s+price|priciest)/i.test(lower))
            extremum = 'expensive';
        return {
    ...
    if (/^(?:remove|delete)\s+/i.test(lower) || /remove\s+.+\s+from\s+(?:my\s+)?cart/i.test(lower)) {
        ...
        const ordMap = { 'first': 0, '1st': 0, 'second': 1, '2nd': 1, 'third': 2, '3rd': 2, 'last': -1 };
        return {
    ...
    const isQtyUpdate = /(?:add\s+another|add\s+one\s+more|make\s+it\s+\d+|make\s+that\s+quantity\s+\d+|change\s+quantity\s+to\s+\d+|update\s+quantity)/i.test(lower);
    if (isQtyUpdate) {
        return {
 ...
 */
function resolveReferences(intent, state, currentCart) {
    var _a, _b;
    ...
    if (intent.type === 'REFINE_SEARCH') {
        ...
        const inherited = Object.assign(Object.assign({}, existingFilters), { maxPrice: ((_a = intent.extractedPrice) === null || _a === void 0 ? void 0 : _a.max) !== undefined ? intent.extractedPrice.max : existingFilters.maxPrice, minPrice: ((_b = intent.extractedPrice) === null || _b === void 0 ? void 0 : _b.min) !== undefined ? intent.extractedPrice.min : existingFilters.minPrice, color: intent.extractedColor || existingFilters.color });
        return {
            resolvedProducts: [],
