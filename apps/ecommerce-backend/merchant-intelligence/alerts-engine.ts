...
export interface BusinessAlert {
  ...
  title: string;
  description: string;
  category: 'INVENTORY' | 'SALES' | 'RETURNS' | 'CUSTOMERS' | 'GROWTH';
  entityName?: string;
  metric: string;
  recommendedAction: string;
  actionType: 'reorder' | 'discount' | 'investigate_returns' | 'promote' | 'review_pricing' | 'none';
  createdAt: string;
}

/**
 * Deterministically generates high-priority actionable operational alerts
 * from live PostgreSQL analytics tables with robust fallback handling.
 */
export async function getBusinessAlerts(): Promise<BusinessAlert[]> {
  ...
  try {
    ...
    console.warn('[Alerts Engine Warning] Stockout alerts query:', err.message);
  }
  ...
  try {
    ...
    const returnRes = await client.query(returnQuery);
    ...
    console.warn('[Alerts Engine Warning] Return anomalies query:', err.message);
  }
  ...
  try {
    ...
    const growthRes = await client.query(growthQuery);
    ...
    console.warn('[Alerts Engine Warning] Growth opportunities query:', err.message);
  }
  ...
  try {
    ...
    const declineRes = await client.query(declineQuery);
    ...
    console.warn('[Alerts Engine Warning] Sales decline query:', err.message);
  }

  // 5. Default General Store Health Info Alert
  alerts.push({
    id: 'alert-store-health-sync',
    severity: 'INFO',
    title: 'Merchant Analytics Ledger Synchronized',
    description: 'All transactions, inventory movements, and customer cohorts are reconciled with PostgreSQL database.',
    category: 'GROWTH',
