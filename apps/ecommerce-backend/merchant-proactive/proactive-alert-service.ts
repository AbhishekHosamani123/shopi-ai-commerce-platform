...
import {
  DetectedBusinessEvent,
  MerchantAiAlertRecord,
  ProactiveAlertSummary,
  AlertStatus
} from './proactive-types';
import { createAction, formatActionPreview } from '../merchant-actions';

function mapRowToAlert(r: any): MerchantAiAlertRecord {
  return {
    alertId: r.alert_id,
...
export async function createOrUpdateAlert(
  event: DetectedBusinessEvent,
  merchantId: string = 'default_merchant'
): Promise<{ alert: MerchantAiAlertRecord; isNew: boolean }> {
  ...
  const existingRes = await client.query(
    `SELECT * FROM merchant_ai_alerts WHERE merchant_id = $1 AND fingerprint = $2 AND status NOT IN ('RESOLVED', 'EXPIRED') LIMIT 1`,
  ...
  if (existingRes.rows.length > 0) {
    ...
    const updateRes = await client.query(
      `UPDATE merchant_ai_alerts 
       SET summary = $1, evidence = $2, recommended_action = $3
       WHERE alert_id = $4
       RETURNING *`,
      [event.summary, JSON.stringify(event.evidence), event.recommendedAction, existing.alert_id]
    );
 ...
 */
export async function getAlertSummary(merchantId: string = 'default_merchant'): Promise<ProactiveAlertSummary> {
  const query = `
    SELECT 
      COUNT(*)::int as total,
...
export async function listAlerts(options: {
  ...
  severity?: string;
  limit?: number;
} = {}): Promise<{ alerts: MerchantAiAlertRecord[]; summary: ProactiveAlertSummary }> {
...
export async function acknowledgeAlert(
  alertId: string,
  merchantId: string = 'default_merchant'
): Promise<MerchantAiAlertRecord | null> {
