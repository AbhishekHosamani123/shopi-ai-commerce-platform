import { PilotModeConfig } from './real-data-types';

const pilotConfigs: Map<string, PilotModeConfig> = new Map();

export class PilotModeGuard {
  /**
   * Initializes or returns Pilot Mode config for a merchant.
   */
  getPilotConfig(merchantId: string = 'default_pilot_merchant'): PilotModeConfig {
   ...
   */
  canExecuteAction(merchantId: string, isExplicitlyApproved: boolean): { allowed: boolean; reason?: string } {
   ...
   */
  recordAiUsage(merchantId: string, tokensUsed: number = 150): PilotModeConfig {
    const cfg = this.getPilotConfig(merchantId);
...
export const pilotModeGuard = new PilotModeGuard();
