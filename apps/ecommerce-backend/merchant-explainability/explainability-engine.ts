...
import { ExplainabilityQuestion, ExplainabilityResponse } from './explainability-types';
...
export class ExplainabilityEngine {
  ...
  async explainDecision(
    ...
    context?: { productId?: number; actionType?: string; targetId?: string },
    ...
    if (context?.productId) {
      const pRes = await client.query('SELECT productid, title, price, discount, stock FROM products WHERE productid = $1', [context.productId]);
      if (pRes.rows.length > 0) prod = pRes.rows[0];
    }
      ...
      case 'WHY_RECOMMENDING':
        return {
          ...
          underlyingTelemetry: {
            ...
            metrics: { currentStock: prod?.stock || 18, nominalLeadTimeDays: 7, empiricalLeadTimeDays: 8.2 }
          },
      ...
      case 'WHAT_HAPPENED_LAST_TIME':
        return {
          ...
          underlyingTelemetry: {
            ...
            metrics: { pastEvaluatedCount: outRes.rows.length, lastErrorPct: parseFloat(latestOutcome?.percentage_error || '4.0') }
          },
          confidenceRating: 'HIGH',
          timestamp: now
        };
