...
import { WhatIfSimulationInput, WhatIfSimulationResult } from './whatif-simulator-types';
import { bayesianPriceElasticityEngine } from '../merchant-learning/price-elasticity/elasticity-engine';
...
export class WhatIfSimulatorEngine {
   ...
   */
  async runSimulation(input: WhatIfSimulationInput): Promise<WhatIfSimulationResult> {
    ...
    switch (input.simulationType) {
      case 'PRICE_CHANGE': {
        ...
        const elastModel = input.productId 
          ? await bayesianPriceElasticityEngine.getOrLearnProductElasticity(input.productId, merchantId)
          : null;
        ...
        return {
          ...
          modelPrediction: {
            ...
            confidence: elastModel?.confidence || 'HIGH',
