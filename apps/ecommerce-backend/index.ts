...
// ── Public campaign banner serving (Evolution/WhatsApp compatibility) ─────────
// The per-recipient personalized promotional banner (same image the EMAIL
// embeds via CID) must be reachable at a PUBLIC URL for the Evolution API to
// attach it to WhatsApp messages. The generator writes banners to
// BANNER_OUTPUT_DIR with unguessable sha256-derived filenames and never
// embeds customer names in filenames, so exact-filename public serving is the
// intended design. Strictly validated to prevent path traversal.
import fs from 'fs';
import path from 'path';
app.get('/campaign-banners/:filename', (req: Request, res: Response) => {
  const filename = String(req.params.filename);
  // Only banner_<24 hex>.png files generated by the banner service.
  if (!/^banner_[a-f0-9]{24}\.png$/.test(filename)) {
    return res.status(404).json({ error: 'Not found' });
  }
  const outputDir = process.env.BANNER_OUTPUT_DIR
