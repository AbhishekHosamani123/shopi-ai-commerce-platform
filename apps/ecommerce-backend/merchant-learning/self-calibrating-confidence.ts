...
export class SelfCalibratingConfidenceEngine {
  /**
  ...
  evaluateConfidence(params: {
    ...
    if (sampleSize >= 100) score += 25;
    else if (sampleSize >= 30) score += 18;
    else if (sampleSize >= 10) score += 8;
    else if (sampleSize < 3) score -= 25;

    // 2. Historical Error Rate (+/- 20 points)
    if (historicalErrorRatePct <= 8) score += 20;
    else if (historicalErrorRatePct <= 15) score += 10;
    else if (historicalErrorRatePct >= 35) score -= 20;
    else if (historicalErrorRatePct >= 25) score -= 10;

    // 3. Data Freshness (+/- 10 points)
    if (dataFreshnessDays <= 2) score += 10;
    else if (dataFreshnessDays > 14) score -= 10;

    // 4. Data Completeness (+/- 10 points)
    if (dataCompletenessPct >= 90) score += 10;
    else if (dataCompletenessPct < 60) score -= 15;

    // 5. Forecast Horizon Degradation
    if (forecastHorizonDays > 30) score -= 10;
    if (forecastHorizonDays > 60) score -= 15;

    // 6. Volatility Penalty
    if (volatilityScore > 2.5) score -= 10;
...
export const selfCalibratingConfidence = new SelfCalibratingConfidenceEngine();
