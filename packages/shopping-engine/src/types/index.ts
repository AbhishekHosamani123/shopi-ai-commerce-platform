...
// ─── ACP Order model (2026-04-17 spec) ───
//
// Orders, refunds and fulfillment are delivered merchant → agent via webhooks
// (`order_create` / `order_update`), never pulled. Refunds are `adjustments[]`
// of `type: 'refund'`; fulfillment is `fulfillments[]`. See ADR 0009.

/** Physical address (shipping / billing / pickup). Spec `Address` schema. */
...
/** Order-level lifecycle status. Unrecognised values MUST be tolerated. */
...
/** 3-field quantity model: original / active / completed. */
export interface AcpOrderLineItemQuantity {
  ordered: number;
  current: number;
  fulfilled?: number;
}

export interface AcpOrderLineItem {
  id: string;
  title: string;
  product_id?: string;
  description?: string;
...
/** Reference to a line item + quantity, used in fulfillments and adjustments. */
export interface AcpLineItemReference {
  id: string;
  quantity: number;
}

export interface AcpEstimatedDelivery {
  earliest: string;
  latest: string;
}

export interface AcpFulfillmentEvent {
  id: string;
  type: string;
  occurred_at: string;
  description?: string;
...
export interface AcpFulfillment {
  ...
  status?: string;
  line_items?: AcpLineItemReference[];
  carrier?: string;
  tracking_number?: string;
  tracking_url?: string;
  destination?: AcpAddress;
  estimated_delivery?: AcpEstimatedDelivery;
  digital_delivery?: {
    access_url?: string;
    license_key?: string;
    expires_at?: string;
  ...
  description?: string;
  events?: AcpFulfillmentEvent[];
}

/** Post-order change: refund, credit, return, exchange, dispute, … */
export interface AcpAdjustment {
  id: string;
  type:
    | 'refund'
    | 'credit'
...
export interface AcpSupportInfo {
  email?: string;
  phone?: string;
  hours?: string;
...
export interface AcpOrder {
  ...
  estimated_delivery?: AcpEstimatedDelivery;
  confirmation?: AcpOrderConfirmation;
  support?: AcpSupportInfo;
  line_items?: AcpOrderLineItem[];
  fulfillments?: AcpFulfillment[];
  adjustments?: AcpAdjustment[];
  totals?: AcpTotal[];
}

/**
 * Webhook event delivered by a merchant. `order_create` for new orders,
 * `order_update` for changes. `data` is always the full Order (never a delta).
 * Unrecognised `type` values MUST be tolerated.
 */
export interface AcpOrderEvent {
  type: 'order_create' | 'order_update' | (string & {});
  data: AcpOrder;
}

/** Structured ACP protocol error body (spec `Error` schema). */
export interface AcpErrorBody {
  type: 'invalid_request' | 'processing_error' | 'service_unavailable' | (string & {});
  code: string;
  message: string;
